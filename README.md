...
- `EV_buy_fund_3`: EV ajustado por fundamento (versão 3; fica disponível no consolidado para análise, mas não é score fixo obrigatório no GA).
...
### Input
- `history_consolidated.csv`.
- O GA **não usa um score fixo único** (não é preso ao `EV_buy_fund_3`). Ele lê `Date`, `ticker`, `open`, `high`, `low`, `close` e usa como candidatos de features as demais colunas numéricas (exceto campos técnicos criados no próprio GA, como `sma200`, `sma200_slope`, `atr`).
...
5. GA recalcula score próprio via walk-forward (a partir das features numéricas disponíveis) e gera relatório final (Excel) e CSV de aplicação dos últimos dias.
