<!-- Imported from: C:\Users\gabri\.codex\worktrees\0b30\test\analysis\optimization_attempts_20260416.md
     SHA1: 8e35342600b97fe4f6558b45b3c010de1727cd28
     Synced at: 2026-04-18T16:33:25+02:00
...
- source: `Attempt 13: Full-metric sweep (take-profit levels neighborhood)`
- promoted change: `partial_take_level: 0.75 -> 1.00`
- acceptance reason: preserved hit-rate priorities while modestly improving fitness and median drawdown (guardrail-safe promotion)
- authoritative checkpoint: [`global_ga_checkpoint.json`](/C:/Users/gabri/.codex/worktrees/0b30/test/global_ga_checkpoint.json)

Current best automated checkpoint vs `git:main` (ref `43679abccf897905919acf63d057096a39170514`):

| Metric | `main` baseline | Current incumbent | Delta |
| --- | ---: | ---: | ---: |
| `fit` | `20.7172` | `21.0214` | `+0.3042` |
| `WR_med_all` | `69.70%` | `70.00%` | `+0.30pp` |
| `WR_target_med_all` | `69.44%` | `69.44%` | `+0.00pp` |
| `mean_alpha_ann` | `-8.27%` | `-8.24%` | `+0.03pp` |
| `alpha_ann_pos_rate` | `19.33%` | `19.33%` | `+0.00pp` |
| `MDD_med` | `11.59%` | `11.22%` | `-0.37pp` |
| `MDD_p75` | `16.15%` | `17.17%` | `+1.03pp` |
| `MDD_duration_med` | `237.0` | `236.5` | `-0.5` |
| `trades_med` | `31.0` | `31.0` | `+0.0` |
| `# >= 70% WR` | `58` | `60` | `+2` |

Reading this in trading terms:

- the incumbent remains guardrail-safe with high hit-rate priorities intact (no material regression in `WR_med_all` / `WR_target_med_all`)
- alpha remains negative overall; this promotion produced only a small improvement in `mean_alpha_ann`
- median drawdown improved vs `main`, but tail-risk (`MDD_p75`) worsened slightly and should be watched
...
| --- | ---: | ---: | ---: |
| `fit` | `21.0068` | `21.0214` | `+0.0146` |
| `WR_med_all` | `70.00%` | `70.00%` | `+0.00pp` |
| `WR_target_med_all` | `69.44%` | `69.44%` | `+0.00pp` |
| `mean_alpha_ann` | `-8.27%` | `-8.24%` | `+0.03pp` |
| `MDD_med` | `11.38%` | `11.22%` | `-0.16pp` |

Why this matters:

- it was a guardrail-safe refinement to the incumbent without sacrificing hit-rate stability
- it suggests the take-profit geometry (`partial_take_level`) can be tuned for small alpha/drawdown gains even after the entry-quality improvements
...
- Telegram send attempted in this environment, but failed due to socket/network permissions (`WinError 10013`)
...
### Attempt 12: Full-metric sweep (signal confirmations + entry threshold) — PROMOTED

Timestamp: `2026-04-18T13:55` local (Europe/Berlin) (promotion applied at ~`14:09`)

Method:

- `py run_local_fullmetric_sweep.py --alpha-focus off --genes "entry_score_threshold,volume_confirm_mode,momentum_confirm_days,regime_threshold,score_strength_scaling,consecutive_loss_cooldown" --combo-budget 8 --seed 101 --out local_fullmetric_sweep_result_20260418_135553.json`
- guardrail refinement: adjusted `run_local_ga_staged._compute_acceptance()` so baseline comparisons **do not require +5pp** WR/WR_target improvements once `origin/main` is already strong (the prior `base + 0.05` floors made promotions effectively impossible after the incumbent was merged into `origin/main`)
- applied via `py apply_fullmetric_sweep_best.py --sweep local_fullmetric_sweep_result_20260418_135553.json` (also updated to recompute the best promotable candidate when `best_promoted` is missing/stale)
...
- changes:
  - `consecutive_loss_cooldown: 6 -> 7`
  - `entry_score_threshold: 0.15 -> 0.20`

Promotion deltas (authoritative from the checkpoint metadata in [`global_ga_checkpoint.json`](/C:/Users/gabri/.codex/worktrees/0b30/test/global_ga_checkpoint.json)):

| Metric | `main` baseline | Prior incumbent | New incumbent | Δ vs prior incumbent |
| --- | ---: | ---: | ---: | ---: |
| `fit` | `20.7172` | `20.7172` | `21.0068` | `+0.2896` |
| `WR_med_all` | `69.70%` | `69.70%` | `70.00%` | `+0.30pp` |
| `WR_target_med_all` | `69.44%` | `69.44%` | `69.44%` | `0.00pp` |
| `mean_alpha_ann` | `-8.27%` | `-8.27%` | `-8.27%` | `-0.00pp` |
| `MDD_med` | `11.59%` | `11.59%` | `11.38%` | `-0.21pp` |

Outcome:

- `promote = true`
- rationale: improved the primary operational metric (`WR_med_all`) without regressing `WR_target_med_all`, kept trades stable, and reduced median drawdown; passed incumbent WR + safety guardrails under the updated “main-baseline non-regression” acceptance rule.

What was learned:

- a slightly stricter entry gating (`entry_score_threshold`) plus a longer loss cooldown can lift WR and reduce drawdown simultaneously *without* collapsing trade counts, suggesting the incumbent still has local “signal quality” headroom even after the `volatility_filter_percentile` and regime-mode safety promotions.

Daily workbook + Telegram status after promotion:

- `py ensure_daily_telegram_file.py` confirmed `summary_latest.xlsx` was already current for store day `2026-04-17`, then attempted to send exactly one document (`summary_latest.xlsx`)
- Telegram send failed in this environment due to socket/network permissions (`WinError 10013`), so delivery could not be confirmed here.

### Attempt 13: Full-metric sweep (take-profit levels neighborhood) â€” PROMOTED

Timestamp: `2026-04-18T15:56` local (Europe/Berlin) (promotion applied at ~`16:13`)

Method:

- `py run_local_fullmetric_sweep.py --alpha-focus off --genes "reward_risk_ratio,partial_take_pct,partial_take_level,partial_take_pct_2,partial_take_level_2,trailing_stop_mode,entry_discount_atr_frac" --combo-budget 12 --seed 131 --out local_fullmetric_sweep_result_20260418_1620_alphaexit.json`
- `py apply_fullmetric_sweep_best.py --sweep local_fullmetric_sweep_result_20260418_1620_alphaexit.json`
...
- change: `partial_take_level: 0.75 -> 1.00` (single-gene step)

Promotion deltas (authoritative from the sweep artifact + post-apply eval-only run):

| Metric | `main` baseline | Prior incumbent | New incumbent | Î” vs prior incumbent |
| --- | ---: | ---: | ---: | ---: |
| `fit` | `20.7172` | `21.0068` | `21.0214` | `+0.0146` |
| `WR_med_all` | `69.70%` | `70.00%` | `70.00%` | `+0.00pp` |
| `WR_target_med_all` | `69.44%` | `69.44%` | `69.44%` | `+0.00pp` |
| `mean_alpha_ann` | `-8.27%` | `-8.27%` | `-8.24%` | `+0.03pp` |
| `MDD_med` | `11.59%` | `11.38%` | `11.22%` | `-0.16pp` |

Outcome:

- `promote = true`
- rationale: kept `WR_med_all` / `WR_target_med_all` stable (no hit-rate regression), improved median drawdown, and slightly improved `mean_alpha_ann` while passing all staged-runner guardrails.

What was learned:

- `partial_take_level` is a useful local refinement lever: raising the first partial take threshold can improve median drawdown (and slightly improve alpha) without damaging hit-rate, but it may worsen tail MDD (`MDD_p75`) and should be monitored in future sweeps.

Daily workbook + Telegram status after promotion:

- ran `py ensure_daily_telegram_file.py` for the latest store day (`2026-04-17`) to ensure [`summary_latest.xlsx`](/C:/Users/gabri/.codex/worktrees/0b30/test/summary_latest.xlsx) delivery
- workbook was already current (`Apply.Date = 2026-04-17`), then a single Telegram send was attempted
- Telegram send failed in this environment due to socket/network permissions (`WinError 10013`), so delivery could not be confirmed here.
